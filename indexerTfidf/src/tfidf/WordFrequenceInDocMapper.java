	...
	public void map(LongWritable key, Text value, Context context)
		...
		while (m.find()) {
			...
			if (!Character.isLetter(matchedKey.charAt(0))
					...
					|| !terms.listTerms.containsKey(matchedKey)) {
				continue;
			}
			valueBuilder.delete(0, valueBuilder.length());
			valueBuilder.append( terms.listTerms.get(matchedKey)); 
			valueBuilder.append("@");
			valueBuilder.append(filesMGR.DocIndice.get(fileName));
			...
			if(!files.contains(fileName)){
				files.add(fileName);
				Iterator<String> it = terms.listTerms.keySet().iterator(); 
				while(it.hasNext()){
					valueBuilder.delete(0, valueBuilder.length());
					valueBuilder.append(terms.listTerms.get(it.next()) );
					valueBuilder.append("@");
					valueBuilder.append(filesMGR.DocIndice.get(fileName));
					INTERM_KEY.set(valueBuilder.toString());
					context.write(INTERM_KEY, ZERO);	
				}
